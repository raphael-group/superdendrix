...
#SBATCH --job-name='MUT.1'
#SBATCH -a 2-768
#SBATCH -N 1
#SBATCH -c 4
#SBATCH --mem=4G
#SBATCH -o /n/fs/ragr-research/projects/sd/gitrepo/superdendrix/experiments/21Q1/gmm/logs_BIC0/MUT.k3.p10000.1.out.%a.txt
#SBATCH -e /n/fs/ragr-research/projects/sd/gitrepo/superdendrix/experiments/21Q1/gmm/logs_BIC0/MUT.k3.p10000.1.err.%a.txt
#SBATCH -t 0-12:00:00
...
#OUTCOMES='data/21Q1/raw/Achilles_gene_dependency.csv' # dep prob

#OUTDIR='experiments/21Q1/'$RUNID'/cycle'$CYCLE'/k'$K'/'$ALT'/'
#OUTDIR='experiments/21Q1/'$RUNID'/final/k'$K'/'$ALT'/'
...
#export GRB_LICENSE_FILE=/usr/local/gurobi/8.0.1/gurobi.lic
...
then
    ...
    #python src/superdendrix.py -t 4 -m $EVENTS -T $OUTCOMES -Tc $G -p $CYCLE -cp $CP -d $D -k $K -nm $NULLMAT -rs $RS -x -curve -o $OUTDIR$G".tsv"  # original SD for FULL MUT
# for ash only
