...
def get_parser():
    ...
    parser.add_argument('-cp', '--cond_it', type=int, default=1000, help='Number of iterations to compute conditional p-value ([-1] = don\'t)')
...
#        print(str(sum(t_opt_arr)/len(t_opt_arr)), "average opt time in sec")
    ...
    if args.output_file:
        of = open(args.output_file, 'w')
        of.write("target\taberrations\t#sample\tscores\tp-value\tz\tmax_score\tz/max_score(%)\tcoverage\tranksum_pval\tbrowser_link\tt_total\tavg_random_z\n")
        of.write(str(args.target_column) + "\t")
        if len(module) > 0:
        ...
        of.write("\t" + str(p_val_str) + "\t" + str(round(zP,4)) + "\t" + str(round(max_score,4)) + '\t' + str(round(100*zP/max_score,4)) + "\t" + str(len(cover)) + "/"+str(N) + "\t")
