...
def get_parser():
    ...
    parser.add_argument('-obj', '--objective', default='exclusivity', choices=['exclusivity', 'cooccurrence1', 'cooccurrence2'], help='objective function')
    # general
...
def run(args):
    ...
    if args.cond_it != -1:
        while len(module) > 1:
            ...
            worst_event, worst_count = conditional_permutation_test(module, events_to_samples, profile, samples, args.cond_it,args)
    ...
    zP = SuperW(cases_by_event, profile, samples,args)
...
#            p_z, p_module, p_best_single, p_cov, p_act_cov, p_act_sample,p_x,p_y,p_mo = optimize_model(genes, samples, w, samples_to_genes, args, seed, logger) #change args.k
#            print("p_Z before fixing: ", p_z)

            # change the global variables
            events_to_samples = p_events_to_samples
            samples_to_genes = p_samples_to_genes
            samples = p_samples
            N = p_N
...
#            print("one optimization after curveball permutation (sec)", t_opt2)
            ...
            ##############
            print(p_samples == samples)
            print(samples_to_genes == p_samples_to_genes)
            p_cases_by_event = [p_events_to_samples[event] for event in p_module]
            n_zP = SuperW(p_cases_by_event, profile, p_samples,args)
            print("coverage", p_cov)
            print("zP from permuted 1: ",p_z)
            print("zP from SuperW fun: ",n_zP)
            ###############
...
#        print(str(sum(t_curve_arr)/len(t_curve_arr)), "average curve time in sec")
    ...
    for i in range(len(module)):
        ...
        scores_ind.append(float(round(SuperW(sampleset,profile,samples,args),2)))
...
'''
def SuperW(cases_by_event, profile, samples, args):
...
'''
def SuperW(cases_by_event, profile, samples, args):
    if args.objective == 'exclusivity':
        C = {}
        for m in cases_by_event:
            for s in m:
                if s in C:
                    C[s] += 1
                else:
                    C[s] = 1
        mut_samples = list(C.keys())
        #print("coverage: ", len(mut_samples))
        scores = list(map(lambda j: profile[j] - (C[j]-1)*abs(profile[j]), mut_samples))
        score = sum(scores)
        return score
#    if args.objective == 'exclusivity':
#        mut_samples = set(s for cases in cases_by_event for s in cases)
...
#        neg = sum(abs(profile[s]) for cases in cases_by_event for s in cases)
#        return 2 * pos - neg
    elif args.objective == 'cooccurrence1':
        C = dict()
        mut_sample_set = set()
        for m in cases_by_event:
            for s in m:
                mut_sample_set.add(s)
                if s in C:
                    C[s] += 1
                else:
                    C[s] = 1
        mut_samples = list(C.keys())
        print("coverage: ", len(mut_samples))
        print("coverage: ", len(mut_sample_set))

        scores = list(map(lambda j: (C[j]-1)*profile[j], mut_samples))
        score = sum(scores)
        return score
    else:
        C = {}
        for m in cases_by_event:
            for s in m:
                if s in C:
                    ...
                    C[s] = 1
        mut_samples = list(C.keys())
        scores = list(map(lambda j: profile[j] * (C[j] - 1), mut_samples))
        score = sum(scores)
        return score

# Conditional permutation test
def conditional_permutation_test(module, eventToCases, profile, samples, num_permutations, args):
        # Compute the score for current module
        cases_by_event = [eventToCases[event] for event in module]
        real_W = SuperW(cases_by_event, profile, samples,args)
        ...
        for i, (event, cases) in enumerate(zip(module, cases_by_event)):
                ...
                for _ in range(num_permutations):
                        ...
                        count += SuperW(cases_by_event, profile, samples,args) >= real_W
...
def build_obj(x, y, w, args):
    if args.objective == 'exclusivity':
        #obj = quicksum(w[j] - (quicksum(x[g] for g in samples_to_genes[j]) - 1)*y[j]*abs(w[j]) for j in samples)
        obj = quicksum((w[j]+abs(w[j]))*y[j] - abs(w[j])*(quicksum(x[g] for g in samples_to_genes[j])) for j in samples)
    elif args.objective == 'cooccurrence1':
        obj = quicksum((-1*w[j])*y[j] + w[j]*(quicksum(x[g] for g in samples_to_genes[j])) for j in samples)
        #obj = quicksum((quicksum(x[g] for g in samples_to_genes[j]) - 1) * w[j]*y[j] for j in samples)
    else:
        obj = quicksum(w[j] * (quicksum(x[g] for g in samples_to_genes[j]) - y[j]) for j in samples)
    return obj
