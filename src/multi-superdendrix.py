...
def get_parser():
    ...
    # specific
    parser.add_argument('-k', '--cardinality', default=-1, type=int, help="Maximum cardinality of event subset [-1]")
    ...
    parser.add_argument('-nsub', '--num_suboptimal', type=int, default=-1, help='number of suboptimal solutions')
    parser.add_argument('-subp', '--p_val_it_sub', type=int, default=-1, help='number of iterations to compute empirical p-value for suboptimal solutions')
    # scoring function
    ...
    parser.add_argument('-x', '--mutex', default=True, action="store_true", help='Penalize coverage overlap on the one-part of the target [False]')
...
#            print("one curveball loading takes (seconds:)",t_curve)
            ...
            random_z_list.append(p_z) # memoize random z
...
#        print(str(sum(t_curve_arr)/len(t_curve_arr)), "average curve time in sec")
    ...
    header_row = "profile\tfeatures\t#sample\tscores\tW(M)/max_score(%)\tcoverage\tranksum_pval\t_opt\tt_total\tP_value\toptimal_order\n"
    rows = []
    row = [args.target_column, ",".join(module), ",".join(cov_ind), ",".join(scores_ind), str(round(100*zP/max_score,4)), str(len(cover))+"/"+str(N), str(ranksum_pval), t_opt, str(time.time() - t_init), str(p_val_str_curve),str(1)]
    rows.append(row)

    # add suboptimal solutions
    
    num_subopt = args.num_suboptimal

    if len(module) > 0:
        prev_module = module
        while num_subopt > 0:
            k = args.cardinality
            for i in prev_module:
                genes.remove(i)
                events_to_samples.pop(i)
                for s in samples_to_genes:
                    if i in samples_to_genes[s]:
                        samples_to_genes[s].remove(i)
            t_init = time.time()
            z, curr_module, best_single, cov, act_cov, act_sample, x, y, mo = optimize_model(genes, samples, w, samples_to_genes, args, seed, logger)
            t_opt = str(time.time() - t_init)
            print("curr_module: ", curr_module, "score: ", z)
            if len(curr_module) == 0:
                row = [args.target_column, "na", "na", "na", "na", "na", "na", t_opt, str(time.time() - t_init), "1", str(args.num_suboptimal-num_subopt+2)]
                break
            else:
                # pruning module
                t_pruning = time.time()
                logger.info("suboptimal now")
                logger.info("initially, len module: " + str(len(curr_module)))
                logger.info("initially, module: " + str(curr_module))
                if args.cond_it != -1:
                    while len(curr_module) > 1:
                        worst_event, worst_count = conditional_permutation_test(curr_module, events_to_samples, profile, samples, args.cond_it)
                        #if args.verbosity: print('worst',worst_event, worst_count/float(args.cond_it))
                        logger.info('worst= ' + str(worst_event) + str(worst_count/float(args.cond_it)))
                        if worst_count/float(args.cond_it) <= (1.0/float(args.cond_it)): break
                        ##if worst_count <= 0: break
                        curr_module.remove(worst_event)

                logger.info('time for pruning(sec) = ' + str((time.time() - t_pruning)))
                logger.info('module after pruning = ' + str(curr_module))

                cases_by_event = [events_to_samples[event] for event in curr_module]
                zP = SuperW(cases_by_event, profile, samples)
                ...
                k = len(curr_module)
                t_s1 = time.time()
                if args.curveball and len(curr_module) > 0:
                    print("curveball")
                    nullmat_dir = args.null_matrices
                    c = 0
                    t_curve_arr = []
                    ...
                    if args.p_val_it_sub > 0:
                        for i in range(args.p_val_it_sub):
                            t_s = time.time()
                            ...
                            t_s = time.time()
                            p_z, p_module, p_best_single, p_cov, p_act_cov, p_act_sample,p_x,p_y,p_mo = optimize_model(p_genes, p_samples, w, p_samples_to_genes, args, seed, logger) #change args.k
                            if p_z >= zP: no_better = no_better + 1
                            if p_z <= zP: no_worse = no_worse + 1
                            p_z_arr_curve.append(p_z)
                            c += 1
                            ...
                            if c == 100:
                                print("100cycle:",str((time.time() - t_s1)/60),"min")
                        p_val_str_curve = str(float(no_better)/args.p_val_it_sub)
                        #p_val_str_curve_2 = str(float(no_worse)/args.p_val_it)
                        print("all cycles:",str((time.time() - t_s1)/60),"min")
                        #avg_p_z_curve = str(float(sum(p_z_arr_curve))/len(p_z_arr_curve))
                        #sd_p_z_curve = str(statistics.stdev(p_z_arr_curve))
            ...
            for i in range(len(curr_module)):
                sampleset = [events_to_samples[curr_module[i]]]
                scores_ind.append(float(round(SuperW(sampleset,profile,samples),2)))
                scores_dict[curr_module[i]] = float(scores_ind[i])
            scores_ind.sort(reverse=True)
            scores_ind = map(str,scores_ind)
            #print(scores_ind)
            curr_module.sort(key=lambda g: float(scores_dict[g]), reverse=True)
            #print(module)
            ...
            cov_ind = []
            for i in range(len(curr_module)):
                cov_ind.append(str(len(events_to_samples[curr_module[i]])))

            for i in range(len(curr_module)):
                cover = cover.union(events_to_samples[curr_module[i]])

            logger.info(args.target_column + "\t" + str(z) + "\t" + str(max_score)  + "\t" + str(z/(max_score+0.0001)) + "\t" + str(cov) + "\t" + str(len(samples)) + "\t" + str(act_cov) + "\t" + str(act_sample) + "\t" + str(obj_norm) + "\t" + str(t_model) + "\t" + str(module) + "\t" + str(len(module)) + "\t" + str(best_single))

            logger.info(", ".join(curr_module))
            ...
            if len(curr_module) == 0:
                ...
                mutsamples = set()
                for i in range(len(curr_module)):
                    mutsamples = mutsamples.union(events_to_samples[curr_module[i]])
                nomutsamples = set(samples) - mutsamples
            ...
            row = [args.target_column, ",".join(curr_module), ",".join(cov_ind), ",".join(scores_ind), str(round(100*zP/max_score,4)), str(len(cover))+"/"+str(N), str(ranksum_pval), t_opt, str(time.time() - t_init), str(p_val_str_curve), str(args.num_suboptimal-num_subopt+2)]
            rows.append(row)
