...
mutations_CERES <- mutations %>% filter(Sample_ID %in% sample_info$DepMap_ID)
length(unique(mutations_CERES$Sample_ID))
...
mutations_all <- mutations_CERES %>% filter(Variant_Classification %in% mutaion_classes)
length(unique(mutations_all$Hugo_Symbol))
